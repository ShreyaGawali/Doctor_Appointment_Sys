...
class Application:
    ...
    def search_db(self):
        ...
        # execute sql
        sql = "SELECT * FROM appointments WHERE name LIKE :input"
        self.res = cur.execute(sql, {'input': '%' + self.input + '%'})
    ...
    def update_db(self):
        ...
        # Convert age and phone to integers
        try:
        ...
        try:
            self.var5 = int(self.var5)
        except ValueError:
            messagebox.showerror("Error", "Invalid phone number. Please enter a valid number.")
            return

        query = "UPDATE appointments SET name=:var1, age=:var2, gender=:var3, location=:var4, phone=:var5, scheduled_time=:var6 WHERE name LIKE :name"
        cur.execute(query,
                    {'var1': self.var1, 'var2': self.var2, 'var3': self.var3, 'var4': self.var4, 'var5': self.var5,
                     'var6': self.var6, 'name': self.input})
        con.commit()
        messagebox.showinfo("Updated", "Successfully Updated.")

    def delete_db(self):
        # delete the appointment
        sql2 = "DELETE FROM appointments WHERE name LIKE :name"
        cur.execute(sql2, {'name': self.namenet.get()})
        con.commit()
